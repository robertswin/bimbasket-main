					...
					<tr id="<?=$arItem["ID"]?>">
						...
						?>
							<td class="control">
								...
								if ($bDeleteColumn):
								?>
									<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>" onclick="ga('send', 'event', 'Корзина', 'Удалить из корзины')"><?=GetMessage("SALE_DELETE")?></a><br />
								<?
								endif;
								if ($bDelayColumn):
								?>
									<a href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>" onclick="ga('send', 'event', 'Корзина', 'Отложить товар', 'Корзина отложить')"><?=GetMessage("SALE_DELAY")?></a>
								<?
								endif;
								?>
							</td>
	...
	<div class="bx_ordercart_order_pay">
		...
		?>
			<div class="bx_ordercart_coupon">
				<span><?=GetMessage("STB_COUPON_PROMT")?></span><input type="text" id="coupon" name="COUPON" value="" onchange="enterCoupon();">&nbsp;<a class="bx_bt_button bx_big" href="javascript:void(0)" onclick="enterCoupon();ga('send', 'event', 'Корзина', 'Применить купон', 'Корзина применить купон')" title="<?=GetMessage('SALE_COUPON_APPLY_TITLE'); ?>"><?=GetMessage('SALE_COUPON_APPLY'); ?></a>
			</div><?
...
?>
		...
		<div class="bx_ordercart_order_pay_center">
			...
			?>
			<a href="javascript:void(0)" onclick="checkOut();ga('send', 'event', 'Корзина', 'Оформить заказ', 'Корзина оформить заказ')" class="checkout-in-cart"><?=GetMessage("SALE_ORDER")?></a>
		</div>
	</div>
</div>
<?
