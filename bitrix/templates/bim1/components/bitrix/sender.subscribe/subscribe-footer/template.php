...
?>
	...
	<form role="form" method="post" action="<?=$arResult["FORM_ACTION"]?>"  onsubmit="BX('bx_subscribe_btn_<?=$buttonId?>').disabled=true;">
		...
		<?if(count($arResult["RUBRICS"])>0):?>
			<div class="bx-subscribe-desc hidden"><?=GetMessage("subscr_form_title_desc")?></div>
		<?endif;?>
		<?foreach($arResult["RUBRICS"] as $itemID => $itemValue):?>
		<div class="bx_subscribe_checkbox_container hidden">
		...
		<div class="bx_subscribe_submit_container">
			<button class="sender-btn btn-subscribe" id="bx_subscribe_btn_<?=$buttonId?>"><span><?=GetMessage("subscr_form_button_footer")?></span></button>
		</div>
	</form>
<?
$frame->beginStub();
