...
$strTitle = (
	isset($arResult['PROPERTIES']['pnyatnoe_nazvanie_for_m']['~VALUE']) && $arResult['PROPERTIES']['pnyatnoe_nazvanie_for_m']['~VALUE'] != ''
	? $arResult['PROPERTIES']['pnyatnoe_nazvanie_for_m']['~VALUE']
	: $arResult['NAME']
);
$strAlt = (
...
if ('' != $arResult['PREVIEW_TEXT'])
{
	...
	{
?>
<div class="item_info_section prew-text">
<?



		echo ('html' == $arResult['PREVIEW_TEXT_TYPE'] ? $arResult['PREVIEW_TEXT'] : '<p class="p-text">'.$arResult['PREVIEW_TEXT'].'</p>');
?>
</div>


<div id="buttons-prod-detail">
						<button class="btn btn-compare compare-button" ><span class="compare"></span><a href="javascript:void(0);" class="" id="<? echo $arItemIDs['COMPARE_LINK']; ?>">В сравнение</a></button>
						
				
						
						<button class="btn btn-to-favorite"><span class="favorits"></span><a href="javascript:void(0)" onclick="add2delay('<?=$arResult["ID"]?>', '<?=$arResult["CATALOG_PRICE_ID_1"]?>',  
   '<?=$arResult["CATALOG_PRICE_1"]?>', '<?=$arResult["NAME"]?>', '<?=$arResult["DETAIL_PAGE_URL"]?>')">Отложить</a></button>
					
					<div class="message-success">
					Товар успешно добавлен в отложенные 
...
<div class="item_info_section">
			...
			<div class="clb">Подарки на сумму до 1200 рублей
в каждом наборе!</div>
			<div class="crb">
			<!-- PRICE -->
...
<?
	...
	{
?>
			<a href="javascript:void(0);" class="bx_big bx_bt_button bx_cart" id="<? echo $arItemIDs['BUY_LINK']; ?>" onclick="ga('send', 'event', 'Страница Товара', 'Купить', 'Купить со страницы товара')"><? echo $buyBtnMessage; ?></a>
<?
	}
	if ($showAddBtn)
	{
?>
			<a href="javascript:void(0);" class="bx_big bx_bt_button bx_cart" id="<? echo $arItemIDs['ADD_BASKET_LINK']; ?>" onclick="ga('send', 'event', 'Страница Товара', 'Добавить в корзину')"><? echo $addToBasketBtnMessage; ?></a>
<?
	}
?>
		</span>
...
{
?>
	<div class="item_buttons vam tar">
		<span class="item_buttons_counter_block" id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" style="display: <? echo ($canBuy ? '' : 'none'); ?>;">
<?
	if ($showBuyBtn)
	{
?>
			<a href="javascript:ga('send', 'event', 'Страница Товара', 'Купить', 'Купить со страницы товара' );javascript:void(0);" class="bx_big bx_bt_button bx_cart" id="<? echo $arItemIDs['BUY_LINK']; ?>" ><? echo $buyBtnMessage; ?></a>
<?
	}
	if ($showAddBtn)
	{
...
?>
	<div class="bx_item_description">
		<div class="bx_item_section_name_gray" style="border-bottom: 1px solid #f2f2f2;"><h1>О товаре</h1></div>
<?
	if ('html' == $arResult['DETAIL_TEXT_TYPE'])
	{
		echo "<p class='detail-2col'>"; echo $arResult['DETAIL_TEXT']; echo"</p>";
	}
	else
	{
		?><p class="detail-2col"><? echo $arResult['DETAIL_TEXT']; ?></p><?
	}
?>
	</div> 
<?
...
?>
 ...
 <div class="prod-in-box" style="display:block;">

<? 
if (is_array($arResult['PROPERTIES']["IN_BOX_PRODUCTS"]["VALUE"]) && count($arResult['PROPERTIES']["IN_BOX_PRODUCTS"]["VALUE"]) > 0) { 
?> 
<h1>Состав набора</h1>
<p class="subtitle">Состав набора может меняться. Пожалуйста, уточняйте точный состав при подтверждении заказа<span class=""></span></p>
<ul class="list-unstyled row"> 
<? 
foreach ($arResult['PROPERTIES']["IN_BOX_PRODUCTS"]["VALUE"] as $komplID) { 
$komplObject = CIBlockElement::GetByID($komplID); 
$komplObject->SetUrlTemplates("/catalog/#SECTION_CODE#/#ELEMENT_CODE#/"); 
$komplObject = $komplObject->GetNext(); 

/*в переменную $renderImage вытаскиваем картинку с уменьшением*/ 
$renderImage = CFile::ResizeImageGet($komplObject["PREVIEW_PICTURE"], Array("width" => 220, "height" => 165), BX_RESIZE_IMAGE_EXACT, false); 

echo '<li class="col-xs-12 col-sm-12 col-md-3 col-lg-3"><div class="caption"><a class="fancybox-media" href="'.$renderImage["src"].'" rel="dop-prod"><img src="'.$renderImage["src"].'" class="thumbnail center-block" /></a><h4>'.$komplObject["NAME"].'</h4><p class="pr-text-dop-tovar">'.$komplObject["~PREVIEW_TEXT"].'</p>'.'</li>';
